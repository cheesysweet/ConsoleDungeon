...
/**
 * HeavyAttack ability used by Warrior, SkeletonWarrior and Lich Lord.
...
public class HeavyAttack extends BaseAbility implements Constants{
    ...
    public boolean execute(int dmg, boolean target){
        return performAbility("%s (-%d AP, -%d Energy)".formatted(this.toString(),
                this.getActionPointCost(), this.getEnergyCost()),
                getAmountOfTargets(), dmg * Constants.SINGLE_TARGET_ABILITY_MULTIPLIER, target);
    }
    ...
    public String toString(){
        return Constants.ABILITY_HEAVY_ATTACK;
    }
}
